...
key.setGlobalKey(['C-x', '<right>'], function (ev) {
                getBrowser().mTabContainer.advanceSelectedTab(1, true);
            }, 'Select next tab');

key.setGlobalKey(['C-x', '<left>'], function (ev) {
                getBrowser().mTabContainer.advanceSelectedTab(-1, true);
            }, 'Select previous tab');

key.setGlobalKey(['C-c', 'c', 'u'], function(ev, arg) {
    command.setClipboardText(window.content.location.href, true);
    display.echoStatusBar("url has been copied into clipboard.", 3000);
}, "Copy current url in clipboard");

key.setGlobalKey(['C-c', 'c', 't'], function(ev, arg) {
    command.setClipboardText(window.content.document.title, true);
    display.echoStatusBar("title has been copied into clipboard.", 3000);
...
key.setEditKey([['C-f'], ['<right>']], function (ev) {
                command.nextChar(ev);
            }, 'Forward char');

key.setEditKey([['C-b'], ['<left>']], function (ev) {
                command.previousChar(ev);
            }, 'Backward char');

key.setEditKey([['M-f'], ['C-<right>']], function (ev) {
                command.forwardWord(ev);
            }, 'Next word');

key.setEditKey([['M-b'], ['C-<left>']], function (ev) {
                command.backwardWord(ev);
            }, 'Previous word');

key.setEditKey([['C-n'], ['<down>']], function (ev) {
                command.nextLine(ev);
            }, 'Next line');

key.setEditKey([['C-p'], ['<up>']], function (ev) {
...
key.setCaretKey([['C-n'], ['<down>']], function (ev) {
                ev.target.ksMarked ? goDoCommand("cmd_selectLineNext") : goDoCommand("cmd_scrollLineDown");
            }, 'Move caret to the next line');

key.setCaretKey([['C-p'], ['<up>']], function (ev) {
                ev.target.ksMarked ? goDoCommand("cmd_selectLinePrevious") : goDoCommand("cmd_scrollLineUp");
            }, 'Move caret to the previous line');

key.setCaretKey([['C-f'], ['<right>']], function (ev) {
                ev.target.ksMarked ? goDoCommand("cmd_selectCharNext") : goDoCommand("cmd_scrollRight");
            }, 'Move caret to the right');

key.setCaretKey([['C-b'], ['<left>']], function (ev) {
                ev.target.ksMarked ? goDoCommand("cmd_selectCharPrevious") : goDoCommand("cmd_scrollLeft");
            }, 'Move caret to the left');

key.setCaretKey([['M-f'], ['C-<right>']], function (ev) {
                ev.target.ksMarked ? goDoCommand("cmd_selectWordNext") : goDoCommand("cmd_wordNext");
            }, 'Move caret to the right by word');

key.setCaretKey([['M-b'], ['C-<left>']], function (ev) {
...
key.setCaretKey('C-l', function (ev) {
                command.recenter(ev);
